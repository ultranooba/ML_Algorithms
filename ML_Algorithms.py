class GaussianNaiveBayes:
	...
	def predict(self, *args):
		if self.data_fitted: # Check if fit method is called first, else, raise exception
			...
			raise Exception("DataNotFittedError : Call fit method before calling predict method to fit the training data to the model")
...
if __name__ == '__main__':
    ...
    c = GaussianNaiveBayes()
    c.fit(Features)
    
    print(c.predict('Cloudy', 'Warm', 'Outdoor'))
    print(c.f1_measure())